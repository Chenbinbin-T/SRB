...
public class LendServiceImpl extends ServiceImpl<LendMapper, Lend> implements LendService {
    ...
    public void creatLend(BorrowInfoApprovalVO borrowInfoApprovalVO, BorrowInfo borrowInfo) {
        ...
        // 计算 年化 / 12
        BigDecimal monthRate = lend.getServiceRate().divide(new BigDecimal(12), 8, BigDecimal.ROUND_DOWN); //保留小数点后8位，向下取整
        // 计算 标的金额 * (年化 / 12 * 期数)
        ...
        lend.setCheckTime(LocalDateTime.now()); //审核时间（现在标的创建出来就直接审核通过）
        lend.setCheckAdminId(1L); //审核人（默认ID为1）
...
//     （4）解冻并扣除投资人资金
        ...
        lendItemList.stream().forEach(item -> {
            ...
            userAccountMapper.updateAccount(investBindCode, new BigDecimal(0), investAmount.negate());// BigDecimal negate方法：返回负值
